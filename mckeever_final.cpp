...
Display( )
{
    ...
    Pattern->SetUniformVariable( (char*)"uBlue",      Blue );
...
float
rand_float(float max_val)
{
    return float(rand()*max_val/RAND_MAX);
}


// return a value for current point in animation cycle, scaled from 0.0 to 1.0
float
GetCycleTime( int ms_in_cycle )
{
...
InitLists( )
{
	...
	glNewList( SphereList, GL_COMPILE );
		...
		//OsuCone( 1.0, 0.0, 1.0, 100.0, 100.0);
	glEndList( );
